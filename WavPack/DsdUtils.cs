...
namespace WavPack
{
	static class DsdUtils
	{
		...
		static int init_dsd_block_high(WavpackStream wps, WavpackMetadata wpmd)
		{
			...
			if (wps.dsd.data.Length - wps.dsd.byteptr < ((flags & Defines.MONO_DATA) > 0 ? 13 : 20))
				return Defines.FALSE;

			rate_i = wps.dsd.data[wps.dsd.byteptr++];
			rate_s = wps.dsd.data[wps.dsd.byteptr++];

			if (rate_s != RATE_S)
				return Defines.FALSE;

			if (wps.dsd.ptable == null)
				wps.dsd.ptable = new int [PTABLE_BINS];

			if (wps.dsd.filters == null)
				wps.dsd.filters = new WavpackStream.DSDfilters[2] { new WavpackStream.DSDfilters(), new WavpackStream.DSDfilters() };

			init_ptable(wps.dsd.ptable, rate_i, rate_s);

			for (channel = 0; channel < ((flags & Defines.MONO_DATA) > 0 ? 1 : 2); ++channel)
			{
				WavpackStream.DSDfilters sp = wps.dsd.filters[channel];

				sp.filter1 = wps.dsd.data[wps.dsd.byteptr++] << (PRECISION - 8);
				sp.filter2 = wps.dsd.data[wps.dsd.byteptr++] << (PRECISION - 8);
				sp.filter3 = wps.dsd.data[wps.dsd.byteptr++] << (PRECISION - 8);
		...
		static long decode_high(WavpackStream wps, int[] output, long sample_count, int bufferStartPos)
		{
			long total_samples = sample_count;
			bool stereo = (wps.wphdr.flags & Defines.MONO_DATA) > 0 ? false : true;
			WavpackStream.DSDfilters[] sp = wps.dsd.filters;
			...
			while (total_samples-- > 0)
			{
				...
				while (bitcount-- > 0)
				{
					int pp = (sp[0].value >> (PRECISION - PRECISION_USE)) & PTABLE_MASK;
					uint split = wps.dsd.low + ((wps.dsd.high - wps.dsd.low) >> 8) * ((uint)wps.dsd.ptable[pp] >> 16);

					if (wps.dsd.value <= split)
					{
						wps.dsd.high = split;
						wps.dsd.ptable[pp] += (UP - wps.dsd.ptable[pp]) >> DECAY;
						sp[0].filter0 = -1;
					}
					else
					{
						wps.dsd.low = split + 1;
						wps.dsd.ptable[pp] += (DOWN - wps.dsd.ptable[pp]) >> DECAY;
						sp[0].filter0 = 0;
					}
					...
					sp[0].bytei = (sp[0].bytei << 1) | (sp[0].filter0 & 1);
					...
					pp = (sp[1].value >> (PRECISION - PRECISION_USE)) & PTABLE_MASK;
					split = wps.dsd.low + ((wps.dsd.high - wps.dsd.low) >> 8) * ((uint)wps.dsd.ptable[pp] >> 16);
					...
					sp[1].bytei = (sp[1].bytei << 1) | (sp[1].filter0 & 1);
